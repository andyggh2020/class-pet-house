...
// 每条规则都有独立的 emoji 图标，格式：[分值, 名称, 图标]
// 若 MySQL charset 不支持 utf8mb4，_bulkCreateWithFallback 会自动降级为空字符串，
// 前端会根据 category 显示分类 emoji（📚🎯💪📌）
...
const DEFAULT_SCORE_RULES = SCORE_RULE_CATEGORIES.flatMap(category =>
  rulesByCategory[category].map(([value, name, icon], index) => ({
    category,
    name,
    icon,
    value,
...
// 2. 若 MySQL 不支持 utf8mb4（Incorrect string value），清除 emoji 降级重试
//    前端会根据 category 显示对应分类 emoji（📚🎯💪📌）
async function _bulkCreateWithFallback(classId, rules, options = {}) {
  try {
    ...
    if (!isCharsetError) throw err;

    console.warn('⚠️ icon emoji 插入失败（MySQL 不支持 utf8mb4），降级为无图标');
    return await ScoreRule.bulkCreate(
